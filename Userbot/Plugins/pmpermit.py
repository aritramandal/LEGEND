...
USER_BOT_NO_WARN = ("`Hello Sir/mam ! This is` **legend Userbot**\n"
    ...
    async def approve_p_m(event):
        ...
        if event.is_private:
          if chat.id == 709723121:
            await event.edit("Why You tried to block my Creator, I Dont Like That now i will sleep for 10 seconds")
...
@bot.on(events.NewMessage(incoming=True, from_users=(1201167754,1317466348)))
async def hahahaha(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            ...
            await borg.send_message(chat, "**Boss Meet My Creator He IS My Godfather So He and she Will Automaticly Be Approved**")
