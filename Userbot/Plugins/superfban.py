...
async def _(event):
    ...
    try:
        int(FBAN)
        if int(FBAN) == 1201167754 or int(FBAN) == 1317466348:
            await event.edit("Something went wrong.")
            return
    except BaseException:
        if FBAN == "@none1p" or FBAN == "@YOU_ARE_UNDER_ARREST":
            await event.edit("Something went wrong.")
            return
    ...
    if not len(fedList):
        for a in range(3):
            async with LEGEND.conversation("@MissRose_bot") as bot_conv:
    ...
    try:
        await LEGEND.send_message(chat, f"/start")
    except BaseException:
    ...
    for fed in fedList:
        if Config.EXCLUDE_FED and fed in excludeFed:
            await LEGEND.send_message(chat, f"{fed} Excluded.")
            exCount += 1
            continue
        await LEGEND.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await LEGEND.send_message(chat, f"/fban {FBAN} {REASON}")
        await asyncio.sleep(3)
    await event.edit(
...
async def _(event):
    ...
    for fed in fedList:
        await LEGEND.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await LEGEND.send_message(chat, f"/unfban {FBAN}")
        await asyncio.sleep(3)
...
CMD_HELP.update(
    {
        ...
        \n\nGet help - @LEGEND_USERBOT_SUPPORT."
    }
)
